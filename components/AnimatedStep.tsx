...
export const AnimatedStep = ({
  ...
  const handleNext = () => {
    setActive((prev) => (prev + 1) % servies.length);
  };

  const handlePrev = () => {
    setActive((prev) => (prev - 1 + servies.length) % servies.length);
  };
  ...
  const randomRotateY = () => {
    return Math.floor(Math.random() * 28) - 10;
  };
  return (
        ...
        <div className="flex justify-between flex-col py-4">
          ...
          >
            ...
            <motion.p className="text-lg text-gray-500 mt-8 dark:text-neutral-300">
              {servies[active].quote.split(" ").map((word, index) => (
                <motion.span
                  key={index}
                  initial={{
                    filter: "blur(10px)",

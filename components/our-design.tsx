...
// import { Plus, Maximize2, ImageIcon, X } from 'lucide-react';
...
//   const displayCards = useMemo(() => {
//     return allCards.slice(0, visibleCount).map((card, index) => ({
//       ...card,
//       className: activeLayout.cardLayouts[index % activeLayout.cardLayouts.length]
//     }));
//   }, [allCards, visibleCount, activeLayout]);
...
const ImageModal = ({ card, isOpen, onClose }: { 
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/90" onClick={onClose}>
      <button 
        onClick={onClose} 
        className="absolute top-4 right-4 w-10 h-10 bg-black/50 hover:bg-black/70 text-white rounded-full p-2 cursor-pointer transition-colors duration-200 z-[100000]"
